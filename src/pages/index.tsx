...
export default function ScholarOS() {
  ...
  // Core Engine (Timer & Hours Tracker)
  useEffect(() => {
    let interval: any;
    if (isActive) {
    ...
    <div className={`min-h-screen ${isGhostMode ? 'bg-black' : 'bg-[#01040a]'} text-white font-sans flex flex-col lg:flex-row overflow-hidden relative transition-colors duration-500`}>
      ...
      <main className="flex-1 overflow-y-auto custom-scrollbar p-4 lg:p-12 pb-24 lg:pb-12">
        <AnimatePresence mode="wait">
            ...
            <motion.div key="h" initial={{opacity:0}} animate={{opacity:1}} className="max-w-6xl mx-auto space-y-8">
              <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-white/5 pb-8">
                <div>
                  ...
                  <p className="text-blue-500 font-black text-[9px] uppercase tracking-[0.4em] mt-2">Active: {name}</p>
                </div>
                <div className="flex gap-8 lg:gap-12 bg-white/5 p-6 rounded-[2rem] border border-white/10">
                  ...
                  <div className="text-right border-l border-white/10 pl-8">
                    ...
                    <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Hours Tracker</p>
                  </div>
                </div>
              </header>

              {/* ALL TASKS (Restored from master menu) */}
              <div className="space-y-12 pb-12">
                <TaskGroup title="01. [cite_start]Core Grind [cite: 2]">
                ...
                <TaskGroup title="02. [cite_start]Subject Exam Power Plays [cite: 2]">
                ...
                <TaskGroup title="03. [cite_start]Bonus Multipliers [cite: 3]">
                ...
                <TaskGroup title="04. [cite_start]Heroic Feats [cite: 3]">
          ...
          {activeTab === 'focus' && (
            <motion.div key="f" initial={{opacity:0, scale:0.95}} animate={{opacity:1, scale:1}} className="max-w-4xl mx-auto h-full flex flex-col items-center justify-center space-y-12">
               <div className="flex bg-white/5 p-2 rounded-2xl border border-white/10">
                  <button onClick={()=>setFocusMode('timer')} className={`px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${focusMode==='timer' ? 'bg-blue-600 text-white' : 'text-slate-500'}`}>Timer Mode</button>
                  <button onClick={()=>setFocusMode('stopwatch')} className={`px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${focusMode==='stopwatch' ? 'bg-blue-600 text-white' : 'text-slate-500'}`}>Stopwatch Mode</button>
               </div>

               <div className="text-center">
                  <h2 className="text-7xl md:text-9xl lg:text-[14rem] font-mono font-black tracking-tighter tabular-nums">
                    {focusMode === 'timer' ? formatTime(timeLeft) : formatTime(stopwatchTime)}
                  </h2>
                  <p className="text-blue-500 font-black text-xs uppercase tracking-[0.5em] mt-4">System Focus Active</p>
               </div>

               <div className="flex flex-wrap justify-center gap-6 items-center">
                  <button onClick={()=>setIsActive(!isActive)} className="px-12 py-5 bg-white text-black font-black uppercase rounded-2xl text-sm tracking-widest shadow-2xl hover:scale-105 active:scale-95 transition-all">
                    {isActive ? 'Pause' : 'Start Session'}
                  </button>
                  <button onClick={()=>{setIsActive(false); focusMode==='timer' ? setTimeLeft(1500) : setStopwatchTime(0)}} className="p-5 bg-white/5 rounded-2xl border border-white/10 text-slate-400 hover:text-white transition-all"><RotateCcw size={24}/></button>
                  <button onClick={()=>setIsGhostMode(true)} className="flex items-center gap-3 px-8 py-5 bg-purple-600/10 text-purple-400 border border-purple-500/20 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-purple-600 hover:text-white transition-all">
          ...
          {activeTab === 'audio' && (
            <motion.div key="au" initial={{opacity:0}} animate={{opacity:1}} className="max-w-4xl mx-auto space-y-12">
              <div className="text-center">
                ...
                <p className="text-slate-500 text-xs font-black uppercase mt-2 tracking-widest">Lo-Fi Frequency Tuner</p>
              </div>
              <div className="grid grid-cols-1 gap-4">
                {LOFI_LIBRARY.map((track, idx) => {
                  const isUnlocked = unlockedTracks.includes(track.id);
                  return (
                    <div key={track.id} className={`p-6 rounded-3xl border flex items-center justify-between transition-all ${currentTrackIdx === idx ? 'bg-blue-600 border-blue-500' : 'bg-white/5 border-white/10'}`}>
                      <div className="flex items-center gap-6">
                        <Disc className={`text-white/40 ${isPlaying && currentTrackIdx === idx ? 'animate-spin' : ''}`} />
                        <div>
                          <p className="text-xs font-black uppercase tracking-widest">{track.name}</p>
                          <p className="text-[10px] text-white/40 mt-1 uppercase">{isUnlocked ? 'Frequency Ready' : `Locked: ${track.cost} SC`}</p>
                        </div>
                      </div>
                      {isUnlocked ? (
                        <button onClick={() => { setCurrentTrackIdx(idx); setIsPlaying(true); }} className="p-4 bg-white/10 rounded-2xl hover:bg-white/20">
                          {currentTrackIdx === idx && isPlaying ? <Pause size={20}/> : <Play size={20}/>}
                        </button>
                      ) : (
      ...
      <AnimatePresence>
        {isGhostMode && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[1000] bg-black flex flex-col items-center justify-center p-8">
             <h1 className="text-[8rem] md:text-[18rem] font-mono font-black opacity-80 tabular-nums">
                {focusMode === 'timer' ? formatTime(timeLeft) : formatTime(stopwatchTime)}
             </h1>
             <div className="flex flex-wrap justify-center gap-8 mt-12">
                ...
                <button onClick={()=>setIsGhostMode(false)} className="px-10 py-6 text-white/20 hover:text-white uppercase text-xs font-black tracking-[0.5em] transition-all">[ EXIT GHOST PROTOCOL ]</button>
             </div>
          </motion.div>
        )}
      </AnimatePresence>
...
function TaskItem({name, sc, icon, onClick, gold}: any) {
  return (
    <motion.div whileHover={{ y: -2 }} className={`p-6 rounded-[2rem] border transition-all flex items-center justify-between gap-4 ${gold ? 'bg-amber-500/5 border-amber-500/20' : 'bg-white/5 border-white/10'}`}>
      <div className="flex items-center gap-4 min-w-0">
        <div className={gold ? 'text-amber-500' : 'text-blue-500'}>{icon}</div>
        <div className="min-w-0">
          <p className="text-[10px] lg:text-xs font-black uppercase tracking-tight text-white/90 truncate">{name}</p>
          <p className="text-emerald-400 text-[10px] font-bold tracking-widest">+{sc} SC</p>
        </div>
      </div>
      <button onClick={onClick} className="shrink-0 px-5 py-3 bg-blue-600 hover:bg-blue-500 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all">Claim</button>
    </motion.div>
  );
}
